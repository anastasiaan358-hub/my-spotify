...
export function AncientResearchPage() {
  const readyCount = additionalAncientDocuments.filter((document) => document.status === 'Готов к карточке').length
    ...
    <div className="ancient-page ancient-research-page">
      <header className="ancient-research-hero">
        ...
        <div className="ancient-research-hero__summary">
          ...
          <dl>
            ...
            <div><dt>Найдено ещё</dt><dd>{additionalAncientDocuments.length}</dd></div>
      ...
      <section className="ancient-research-section" aria-labelledby="additional-documents-heading">
        <header className="ancient-section-heading">
          ...
          <p>
            {additionalAncientDocuments.length} нотированных позиций из академического указателя пока отсутствуют в основном каталоге. {readyCount} из них уже имеет устойчивое название и атрибуцию; остальные должны оставаться карточками фрагментов.
          </p>
        </header>
        <div className="ancient-document-register" role="list">
          {additionalAncientDocuments.map((document, index) => (
      ...
      <section className="ancient-research-section" aria-labelledby="bibliography-heading">
        <header className="ancient-section-heading">
          <div>
            <span className="mono-label">03 / WORKING BIBLIOGRAPHY</span>
