...
- **Celery** — транскодирование (отдельная нода, CPU-bound), индексация поиска, агрегации, beat-расписание.
...
CREATE TABLE users (
    ...
    username          VARCHAR(32) NOT NULL UNIQUE,  -- всегда lowercase: регистр не различается
...
**Идентификатор — логин, а не email.** Почтового контура в MVP нет: ни верификации адреса, ни сброса пароля письмом, ни самого поля. Логин — 3–32 символа латиницы, цифр, точки, дефиса и подчёркивания, в базе всегда в нижнем регистре. Кириллица запрещена намеренно: латинская `a` и кириллическая `а` дали бы визуально неразличимые аккаунты, а NFKC их не схлопывает. Цена решения зафиксирована честно — у забытого пароля не остаётся ни одного канала восстановления, кроме рук админа, пока не появится 2FA с recovery-кодами (см. последний раздел). Email вернётся, когда понадобится по делу, и уже как необязательное поле профиля, а не как идентификатор.
...
|---|---|---|---|
| `default` | services-1 | 2 × concurrency 8 | запись playback-событий и search_history, офлайн-ревалидация, ночная чистка отозванных токенов |
...
```json
{
  "error": {
    ...
    "details": {"username": ["Пользователь с таким логином уже существует."]},
...
**Возврат доступа и второй фактор.** Почтовый контур убран из MVP вместе с email как идентификатором (§5.2), и с ним исчез единственный канал восстановления: забытый пароль сейчас сбрасывает только админ. Закрывает это двухфакторная аутентификация — TOTP плюс recovery-коды, которые без почты перестают быть приятным дополнением и становятся основным способом вернуть аккаунт. Требует промежуточного шага в выдаче токенов (challenge-токен → подтверждение кодом → пара access/refresh), отдельного троттлинга на шестизначный код и политики на случай «потеряны и телефон, и коды». Обязательная 2FA для staff-аккаунтов админки (§7.11) — отдельный и более ранний шаг.

**Email, когда он понадобится.** Вернуть почту заставят два триггера: фискальный чек по 54-ФЗ при платежах и запасной канал восстановления доступа. Тогда это будет необязательное подтверждаемое поле профиля, и к нему понадобится всё то, что и так не было выбрано: SMTP-провайдер (Postmark/SES/Mailgun — по доставляемости в российские почтовики), шаблоны, DKIM/SPF/DMARC и обработка bounce/complaint (вебхук → пометка адреса недоставляемым).
